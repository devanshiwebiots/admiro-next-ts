...
import { HoverableRowBody, HoverableRowData, HoverableRowHead } from "@/Data/Form&Table/Table/ReactstrapTable/BasicTableData";
        ...
        <CommonTable tableClass="signal-table" headData={HoverableRowHead} hover>
          {HoverableRowBody.map((data) => (
            <tr key={data.id}>
              ...
              <td className="d-flex align-items-center">{data.icon}<span className={`font-${data.color}`}>{data.status}</span></td>
