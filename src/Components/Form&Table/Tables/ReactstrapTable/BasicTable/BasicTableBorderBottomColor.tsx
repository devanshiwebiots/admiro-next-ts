...
import { BasicTableBody, BasicTableBorderColor, BasicTableHead } from "@/Data/Form&Table/Table/ReactstrapTable/BasicTableData";
        ...
        <CommonTable headData={BasicTableHead} headRowClass="border-bottom-primary">
          {BasicTableBody.map((data) => (
            <tr className={`border-bottom-${data.color}`} key={data.id}>
              ...
              <td><Image width={30} height={30} className="img-30 me-2" src={`${ImagePath}/${data.image}`} alt="users" />{data.firstname}</td>
              <td>{data.lastName}</td>
              <td>{data.userName}</td>
