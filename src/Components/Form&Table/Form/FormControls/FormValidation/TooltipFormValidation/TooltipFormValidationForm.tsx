import { City, CityFeedback, FirstName, LastName, LooksGood, State, StateChoose, StateFeedback, SubmitForm, UserName, UsernameFeedback, Zip, ZipFeedback } from "@/Constant";
import { TooltipValidationStateList } from "@/Data/Form&Table/Form/FormControls";
...
const TooltipFormValidationForm:React.FC<TooltipFormValidationProp>=({errors,submitErrors,setSubmitError}) => {
  return (
    <Form>
      <Row className="g-3 custom-input">
        ...
        <Col md="4" className="position-relative">
          ...
          <Field name="lastname" type="text" className={`form-control ${submitErrors && `${errors.lastName ? "is-invalid" : "is-valid"}`}`} placeholder={"Otto"} />
          <FormFeedback valid tooltip>{LooksGood}</FormFeedback>
        </Col>
        <Col md="4" className="position-relative">
