...
import { LanguagesData } from "@/Data/Layout/HeaderData";
  ...
  return (
    <li className="custom-dropdown d-sm-block d-none">
      <div className={`translate_wrapper ${langDropdown ? "active" : ""}`}>
        ...
        <ul className={`custom-menu profile-menu language-menu py-0 more_lang ${langDropdown ? "active" : ""}`}>
          {LanguagesData?.map((item, i) => (
            <li className="d-block" key={i} onClick={() => changeLanguage(item)}>
              <a className="text-decoration-none" data-lng={item.title} href={Href}>
